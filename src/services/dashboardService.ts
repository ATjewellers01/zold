import prisma from "../config/db.js";

export const getDashboardMetricsService = async () => {
  const totalUsers = await prisma.user.count();

  const usersByRole = await prisma.user.groupBy({
    by: ["role"],
    _count: true,
  });

  const walletsSum = await prisma.inventory.aggregate({
    _sum: {
      goldBalance: true,
      pledgedGold: true,
      rupeeBalance: true,
    },
  });

  const freeGold =
    (Number(walletsSum._sum.goldBalance) || 0) -
    (Number(walletsSum._sum.pledgedGold) || 0);

  const today = new Date();
  today.setHours(0, 0, 0, 0);

  const todayTransactions = await prisma.metalTransaction.aggregate({
    where: {
      createdAt: { gte: today },
      status: "COMPLETED",
    },
    _count: true,
    _sum: { finalAmount: true, metalGrams: true },
  });

  const todayBuy = await prisma.metalTransaction.aggregate({
    where: {
      createdAt: { gte: today },
      transactionType: "BUY",
      status: "COMPLETED",
    },
    _sum: { finalAmount: true },
  });

  const todaySell = await prisma.metalTransaction.aggregate({
    where: {
      createdAt: { gte: today },
      transactionType: "SELL",
      status: "COMPLETED",
    },
    _sum: { finalAmount: true },
  });

  const weekAgo = new Date();
  weekAgo.setDate(weekAgo.getDate() - 7);

  const weekTransactions = await prisma.metalTransaction.aggregate({
    where: {
      createdAt: { gte: weekAgo },
      status: "COMPLETED",
    },
    _count: true,
    _sum: { finalAmount: true },
  });

  const monthAgo = new Date();
  monthAgo.setMonth(monthAgo.getMonth() - 1);

  const monthTransactions = await prisma.metalTransaction.aggregate({
    where: {
      createdAt: { gte: monthAgo },
      status: "COMPLETED",
    },
    _count: true,
    _sum: { finalAmount: true },
  });

  const pendingKYC = await prisma.kyc.count({
    where: { status: "PENDING" },
  });

  const approvedKYC = await prisma.kyc.count({
    where: { status: "APPROVED" },
  });

  const activePartners = await prisma.partner.count({
    where: { isActive: true },
  });

  const currentRate = await prisma.metalRate.findFirst({
    where: { metal: "GOLD", isActive: true },
    orderBy: { createdAt: "desc" },
  });

  const recentTransactions = await prisma.metalTransaction.findMany({
    take: 10,
    orderBy: { createdAt: "desc" },
    include: {
      user: {
        select: {
          name: true,
          email: true,
          phone: true,
        },
      },
    },
  });

  return {
    overview: {
      totalUsers,
      usersByRole,
      totalGoldHoldings: Number(walletsSum._sum.goldBalance) || 0,
      pledgedGold: Number(walletsSum._sum.pledgedGold) || 0,
      freeGold: Number(freeGold),
      totalRupeeBalance: Number(walletsSum._sum.rupeeBalance) || 0,
      pendingKYC,
      approvedKYC,
      activePartners,
    },
    transactions: {
      today: {
        count: todayTransactions._count,
        volume: Number(todayTransactions._sum.finalAmount) || 0,
        goldVolume: Number(todayTransactions._sum.metalGrams) || 0,
        buyVolume: Number(todayBuy._sum.finalAmount) || 0,
        sellVolume: Number(todaySell._sum.finalAmount) || 0,
      },
      week: {
        count: weekTransactions._count,
        volume: Number(weekTransactions._sum.finalAmount) || 0,
      },
      month: {
        count: monthTransactions._count,
        volume: Number(monthTransactions._sum.finalAmount) || 0,
      },
    },
    currentGoldRate: currentRate
      ? {
          buyRate: Number(currentRate.buyRate),
          sellRate: Number(currentRate.sellRate),
          updatedAt: currentRate.createdAt,
        }
      : null,
    recentTransactions,
  };
};

export const getTransactionAnalyticsService = async (period: string) => {
  const days = period === "30d" ? 30 : period === "90d" ? 90 : 7;
  const startDate = new Date();
  startDate.setDate(startDate.getDate() - days);
  startDate.setHours(0, 0, 0, 0);

  const transactions = await prisma.metalTransaction.findMany({
    where: {
      createdAt: { gte: startDate },
      status: "COMPLETED",
    },
    select: {
      transactionType: true,
      finalAmount: true,
      metalGrams: true,
      createdAt: true,
    },
    orderBy: { createdAt: "asc" },
  });

  const dailyData: Record<string, any> = {};

  transactions.forEach((txn) => {
    const date = txn.createdAt.toISOString().split("T")[0];

    if (!dailyData[date]) {
      dailyData[date] = {
        date,
        count: 0,
        volume: 0,
        buyCount: 0,
        sellCount: 0,
        buyVolume: 0,
        sellVolume: 0,
        goldVolume: 0,
      };
    }

    dailyData[date].count++;
    dailyData[date].volume += Number(txn.finalAmount);
    dailyData[date].goldVolume += Number(txn.metalGrams);

    if (txn.transactionType === "BUY") {
      dailyData[date].buyCount++;
      dailyData[date].buyVolume += Number(txn.finalAmount);
    } else {
      dailyData[date].sellCount++;
      dailyData[date].sellVolume += Number(txn.finalAmount);
    }
  });

  return Object.values(dailyData);
};

export const getUserGrowthAnalyticsService = async (period: string) => {
  const days = period === "90d" ? 90 : 30;
  const startDate = new Date();
  startDate.setDate(startDate.getDate() - days);

  const users = await prisma.user.findMany({
    where: {
      createdAt: { gte: startDate },
    },
    select: {
      createdAt: true,
      role: true,
    },
    orderBy: { createdAt: "asc" },
  });

  const dailyGrowth: Record<string, any> = {};

  users.forEach((user) => {
    const date = user.createdAt.toISOString().split("T")[0];

    if (!dailyGrowth[date]) {
      dailyGrowth[date] = {
        date,
        newUsers: 0,
        newAdmins: 0,
        newPartners: 0,
      };
    }

    dailyGrowth[date].newUsers++;
    if (user.role === "ADMIN") dailyGrowth[date].newAdmins++;
    if (user.role === "PARTNER") dailyGrowth[date].newPartners++;
  });

  return Object.values(dailyGrowth);
};
